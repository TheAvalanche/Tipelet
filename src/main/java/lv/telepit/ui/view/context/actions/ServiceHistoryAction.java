...
public class ServiceHistoryAction extends AbstractAction<ServiceView> {
    ...
    @Override
    public void execute() {
        List<ChangeRecord> historyList = view.getUi().getServiceGoodService().findChanges(good);
        ...
        StreamResource pdfStream = getPDFStream(view.getUi().getServiceGoodService().findChanges(good));
