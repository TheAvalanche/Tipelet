...
public class PdfUtils {
	...
	public void createBill(ServiceGood serviceGood) throws DocumentException, IOException {
		...
		ColumnText.showTextAligned(writer.getDirectContent(), Element.ALIGN_CENTER, new Phrase(StringUtils.defaultIfEmpty(serviceGood.getCustomId(), "")), 250, 820, 0);
	...
	public void createInvoice(BusinessReceipt businessReceipt) throws DocumentException, IOException {
		...
		receiverTable.addCell(noBCell("E-pasts", normalFont));
		receiverTable.addCell(noBCell(businessReceipt.getReceiverMail(), boldFont));
